...
contract Word is ERC721, ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    constructor(address _mineAddress) ERC721("Word", "WD") {
    ...
    function mint(string memory word) public payable {
        ...
        require(msg.value == mintPrice, "eth illegal!");
        ...
        unchecked {
            ...
            for (uint256 i = 0; i < words.length; ++i) {
                ...
                if (!isWordLock(wordHash)) {
                    ...
                    if (wordTokenID != 0){
                        sentenceMintRevenue[wordTokenID] += sentenceWordPrice;
                    }
                    else{
                        mineAmount += sentenceWordPrice;
                    }
                }
        ...
        returns (string memory)
    {
        ...
        return Descriptor.GetWordDesc(queryWord(tokenId));
    }
